...
static int check_is_digit(char c)
{
    return (c >= '0' && c <= '9');
}
...
static int count_words_in_str(char const *str)
{
    ...
    for (int i = 0; str[i]; i++) {
        ...
        || check_is_digit(str[i])) && !same_word) {
            same_word = 1;
            count++;
        }
...
static int get_word_length(char const *str, int i)
{
    int len = 0;
...
static int copy_word(char **word_array, char const *str, int i, int word_index)
{
